...
onsubmit="U()">Installed version: SR 0.13.3-dev<br>Download the latest binary: 
...
var ws,c=document.getElementById("liveviewCanvas"),ctx=null,pPL=0,lOf=0,mW=0,mH=0,renderer=null,scene=null,camera=null,mD=0;function updatePreview(e){if(mW=e[3],mH=e[4],mD=e[5],1==e[2])for(ctx||(c.width=.98*window.innerWidth,c.height=.98*window.innerHeight,ctx=c.getContext("2d"),pPL=Math.min(c.width/mW,c.height/mH),lOf=Math.floor((c.width-pPL*mW)/2)),l=6;l<e.length;l+=3){let r=(l-6)/3;ctx.fillStyle=`rgb(${e[l]},${e[l+1]},${e[l+2]})`,ctx.beginPath(),ctx.arc(r%mW*pPL+.5*pPL+lOf,Math.floor(r/mW)*pPL+.5*pPL,.4*pPL,0,2*Math.PI),ctx.fill()}else if(2==e[2]){if(!renderer){c.width=0,c.height=0,(renderer=new THREE.WebGLRenderer({alpha:!0})).setClearAlpha(0),renderer.setClearColor(0,0),renderer.setSize(.98*window.innerWidth,.98*window.innerHeight),document.body.appendChild(renderer.domElement),(camera=new THREE.PerspectiveCamera(45,window.innerWidth/window.innerHeight,1,500)).position.set(0,0,100),camera.lookAt(0,0,0),(scene=new THREE.Scene).background=null;for(var r=-5*(mW-1)/2,n=-5*(mH-1)/2,t=-5*(mD-1)/2,o=0;o<mW;o++)for(var i=0;i<mH;i++)for(var a=0;a<mD;a++){const e=new THREE.SphereGeometry(1,32,16),c=new THREE.MeshBasicMaterial,l=new THREE.Mesh(e,c);l.position.set(r+5*o,n+5*i,t+5*a),scene.add(l)}}let d=6;var l=1;for(o=0;o<mW;o++)for(i=0;i<mH;i++)for(a=0;a<mD;a++)l<scene.children.length&&(scene.children[l].material.color=new THREE.Color(""+e[3*l+d]/255,""+e[3*l+d+1]/255,""+e[3*l+d+2]/255)),l++;scene.rotation.x+=.01,scene.rotation.y+=.01,renderer.render(scene,camera)}}try{ws=top.window.ws}catch(e){}ws&&ws.readyState===WebSocket.OPEN?ws.send("{'lv':true}"):(ws=new WebSocket(("https:"==window.location.protocol?"wss":"ws")+"://"+document.location.host+"/ws")).onopen=()=>{ws.send("{'lv':true}")},ws.binaryType="arraybuffer",ws.addEventListener("message",e=>{try{if("[object ArrayBuffer]"===toString.call(e.data)){let e=new Uint8Array(event.data);if(76!=e[0])return;updatePreview(e)}}catch(e){console.error("Peek WS error:",e)}})
</script></body></html>)=====";
