...
Installed version: 0.13.2-b1<br>Download the latest binary: <a 
...
content="width=device-width,initial-scale=1,minimum-scale=1"><meta 
charset="utf-8"><meta name="theme-color" content="#222222"><title>
WLED Live Preview</title><style>body{margin:0}</style></head><body><div 
style="margin:auto"><canvas id="liveviewCanvas" width="320" , height="320" 
style="border:1px solid #000">LiveView</canvas></div><script>
var ws,canvas=document.getElementById("liveviewCanvas"),leds="",matrixWidth=0,pixelsPerLed=0;if(canvas.getContext){var ctx=canvas.getContext("2d");function drawLedCircles(e,t,i,n,s,r){ctx.fillStyle=`rgb(${i},${n},${s})`,ctx.beginPath(),ctx.arc(e*pixelsPerLed+pixelsPerLed/2,t*pixelsPerLed+pixelsPerLed/2,pixelsPerLed/2,0,2*Math.PI),ctx.fill()}function paintLeds(){for(i=2;i<leds.length;i+=3){let e=(i-2)/3;drawLedCircles(e%matrixWidth,Math.floor(e/matrixWidth),leds[i],leds[i+1],leds[i+2],255)}}}function updatePreview(e){leds=e,matrixWidth=Math.sqrt((leds.length-2)/3),pixelsPerLed=canvas.width/matrixWidth,paintLeds()}function getLiveJson(e){try{if("[object ArrayBuffer]"===toString.call(e.data)){let e=new Uint8Array(event.data);if(76!=e[0])return;updatePreview(e)}}catch(e){console.error("Peek WS error:",e)}}try{ws=top.window.ws}catch(e){}ws&&ws.readyState===WebSocket.OPEN?(console.info("Peek uses top WS"),ws.send("{'lv':true}")):(console.info("Peek WS opening"),(ws=new WebSocket(("https:"==window.location.protocol?"wss":"ws")+"://"+document.location.host+"/ws")).onopen=function(){console.info("Peek WS open"),ws.send("{'lv':true}")}),ws.binaryType="arraybuffer",ws.addEventListener("message",getLiveJson)
</script></body></html>)=====";
